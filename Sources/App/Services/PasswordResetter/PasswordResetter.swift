...
struct PasswordResetter {
    ...
    func reset(for user: UserAccountModel) async throws {
        ...
        let content = BrevoMail(
            sender: .init(
                name: "Zenix Invest",
                email: "noreply@zenix-invest.com"
            ),
            to: [.init(
                name: user.fullName,
                email: user.email
            )],
            subject: "Verify your Zenix account",
            htmlContent: template(token: resetPasswordToken.value)
        )
    ...
    private var mailURI: URI {
        URI(string: "https://api.brevo.com/v3/smtp/email")
    }
}
...
private func template(token: String) -> String {
"""
<!DOCTYPE html>
<html>
<head>
                    ...
                    <table border="0" cellpadding="0" cellspacing="0">
                      <tr>
                        <td align="center" bgcolor="#1a82e2" style="border-radius: 6px;">
                          <a href="\(Environment.baseURL)/reset-password?token=\(token)" target="_blank" style="display: inline-block; padding: 16px 36px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; color: #ffffff; text-decoration: none; border-radius: 6px;">Verify email address</a>
                        </td>
                      </tr>
                    </table>
                  </td>
